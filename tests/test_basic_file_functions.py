...
class TestFileTools(unittest.TestCase):
    ...
    def test_w_and_r(self):
        ...
        write.invoke({"path": "test.txt", "content": "Hello World"})
        result = read.invoke({"path": "test.txt"})
        self.assertEqual(result, "Hello World")

    def test_a(self):
        """Test appending to a file using LangChain tools."""
        write.invoke({"path": "test.txt", "content": "Line 1\n"})
        append.invoke({"path": "test.txt", "content": "Line 2\n"})
        result = read.invoke({"path": "test.txt"})
        self.assertEqual(result, "Line 1\nLine 2\n")

    def test_c(self):
        """Test clearing a file using LangChain tools."""
        write.invoke({"path": "test.txt", "content": "Some content"})
